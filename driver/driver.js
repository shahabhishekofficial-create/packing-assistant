...
$("invoiceInput").onchange=async e=>{const input=e.target,file=input.files[0],outletId=input.dataset.outletId,mode=input.dataset.mode||"invoice",itemId=input.dataset.itemId||"";input.value="";if(!file||!outletId)return;if(mode==="invoice"&&!/^\d+$/.test(invoiceNumber))return toast("Enter the invoice number before uploading.","error");if(!file.type.startsWith("image/"))return toast("Please select an image.","error");if(file.size>15*1024*1024)return toast("Image must be under 15 MB.","error");const key=String(outletId);ds.busy[key]=true;render();try{const liveOutlet=ds.outlets.find(o=>String(o.outlet_id)===key);if(!liveOutlet)throw new Error("Outlet is no longer assigned to this driver. Refresh and try again.");const prepared=await compressImage(file);const action=mode==="rejected"?"rejection_photo_url":"upload_url";const d=await api(action,{order_id:ds.orderId,outlet_id:outletId,outlet_name:(liveOutlet.outlet_name||""),item_id:itemId,filename:prepared.name,mime_type:prepared.type,extension:"jpg",invoice_number:invoiceNumber});const {error}=await getSB().storage.from(mode==="rejected"?"delivery-evidence":"delivery-invoices").uploadToSignedUrl(d.path,d.token,prepared,{contentType:prepared.type});if(error)throw error;if(mode==="rejected"){await api("save_rejection_photo",{order_id:ds.orderId,outlet_id:outletId,item_id:itemId,path:d.path,filename:prepared.name,mime_type:prepared.type});toast("Damage photo uploaded.","success");}else{await api("invoice_uploaded",{order_id:ds.orderId,outlet_id:outletId,path:d.path,invoice_number:invoiceNumber,filename:(liveOutlet.outlet_name||"Outlet")+" - "+invoiceNumber+".jpg",mime_type:prepared.type});const current=ds.outlets.find(o=>String(o.outlet_id)===String(d.outlet_id||outletId));if(current){current.delivery=current.delivery||{};current.delivery.invoice_path=d.path;current.delivery.invoice_number=invoiceNumber;current.delivery.invoice_uploaded_at=new Date().toISOString();current.delivery.status="pending";}toast("Invoice uploaded successfully.","success");}}catch(err){console.error("Driver evidence upload failed",{mode,outletId,itemId,message:err?.message||String(err)});toast("Upload failed: "+(err?.message||"Please try again."),"error");}finally{delete ds.busy[key];render();focusOutlet(outletId,true);}};async function saveRejections(outletId){
 const outlet=ds.outlets.find(o=>String(o.outlet_id)===String(outletId)); if(!outlet)return;
