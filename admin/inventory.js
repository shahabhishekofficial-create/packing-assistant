...
function validateItem(p,existing=[]){
 ...
 if(p.count_mode==="packet"&&(!p.count_unit||p.count_unit.length>30))e.push("Count unit (Box, Bag, Carton, etc.) is required for pack/container mode.");if(p.count_mode==="packet"&&(!Number.isFinite(p.default_pack_size)||p.default_pack_size<=0))e.push("Quantity per count unit is required and must be greater than 0.");if(p.count_mode==="unit"&&p.default_pack_size!=null)e.push("Quantity per count unit is only used for pack/container mode.");
 ...
 p.barcodes.forEach(b=>{if(existing.some(x=>x.id!==S.editId&&(x.barcodes||[]).includes(b)))e.push("Barcode already linked: "+b)});
 return e;
}
...
function renderItems(){
 ...
 $("masterSummary").textContent=S.section==="restaurant" ? S.items.filter(i=>i.active).length+" active · "+S.items.filter(i=>!i.active).length+" inactive · "+new Set(S.items.flatMap(i=>i.barcodes||[])).size+" barcodes" : S.items.filter(i=>i.active).length+" active · "+S.items.filter(i=>!i.active).length+" inactive";
 $("itemList").innerHTML=arr.map(i=>'<article class="adminItemRow '+(i.active?"":"inactiveItem")+'"><div class="adminItemMain"><b>'+esc(i.name)+'</b><span>'+esc(i.category)+' · '+esc(i.base_uom)+' · '+esc(i.count_mode)+(i.default_pack_size!=null?" · pack "+esc(i.default_pack_size)+" "+esc(i.base_uom):"")+(i.storage_shelf||i.storage_rack?" · "+esc([i.storage_shelf?"Shelf "+i.storage_shelf:"",i.storage_rack?"Rack "+i.storage_rack:""].filter(Boolean).join(" / ")):"")+'</span><small>'+(i.barcodes.length?"Barcode: "+esc(i.barcodes.join(" | ")):"No barcode")+(i.brand?" · "+esc(i.brand):"")+'</small></div><div class="adminItemActions"><span class="statusPill '+(i.active?"on":"off")+'">'+(i.active?"Active":"Inactive")+'</span><button class="secondary editItem" data-id="'+i.id+'">Edit</button>'+(i.active?'<button class="dangerBtn deactivateItem" data-id="'+i.id+'">Deactivate</button>':"")+'</div></article>').join("")||'<div class="emptyState">No items found.</div>';
...
function resetForm(){S.editId=null;$("dialogTitle").textContent="Add Restaurant Item";$("itemForm").reset();$("itemBarcodes").disabled=false;$("dialogFetchBtn").disabled=!norm($("itemBarcodes").value);$("itemMode").value="";$("itemPackSize").value="";$("itemPackSize").disabled=true;$("itemCountUnit").value="";$("countUnitWrap").classList.add("hidden");$("packSizeWrap").classList.add("hidden");$("fieldErrors").innerHTML="";$("dialogFetchStatus").textContent="Data is placed into these fields automatically."}
async function openDialog(id=null){resetForm();try{await loadCategories()}catch(e){alertBox(e.message||"Could not load categories.","error");return}if(id){const i=S.items.find(x=>x.id===id);if(!i)return;S.editId=id;$("dialogTitle").textContent="Edit Item";setCategoryValue(i.category);$("itemName").value=i.name;$("itemUom").value=i.base_uom;$("itemMode").value=i.count_mode;$("itemCountUnit").value=i.count_unit||"";$("itemPackSize").value=i.default_pack_size??"";$("itemPackSize").disabled=i.count_mode!=="packet";$("countUnitWrap").classList.toggle("hidden",i.count_mode!=="packet");$("packSizeWrap").classList.toggle("hidden",i.count_mode!=="packet");$("itemBarcodes").value=i.barcodes.join(" | ");$("itemNoBarcode").checked=!!i.no_barcode;$("itemAliases").value=(i.aliases||[]).join(" | ");$("itemBrand").value=i.brand||"";$("itemShelf").value=i.storage_shelf||"";$("itemRack").value=i.storage_rack||""} $("itemDialog").showModal()}
...
function validateVegetable(p,existing=[]){const e=[];if(!p.name)e.push("Vegetable name is required.");if(!p.category)e.push("Category is required.");if(!["kg","pcs"].includes(p.base_uom))e.push("Base UOM must be kg or pcs.");const same=existing.find(x=>x.id!==S.editId&&x.section==="vegetable"&&norm(x.name).toLowerCase()===p.name.toLowerCase());if(same)e.push("Duplicate vegetable name.");return e}
async function saveVegetable(e,keepOpen=false){e?.preventDefault();const p=vegetablePayload(),errors=validateVegetable(p,S.items);$("vegetableErrors").innerHTML=errors.map(x=>"<div>"+esc(x)+"</div>").join("");if(errors.length)return;const b=$("vegetableSaveBtn");b.disabled=true;b.textContent="Saving…";try{const{data,error}=await db.rpc("inv_v2_save_item_v5",{p_session_token:token(),p_item_id:S.editId,p_payload:p,p_import_id:null});if(error)throw error;const msg=S.editId?"Vegetable updated.":"Vegetable saved.";if(keepOpen){resetVegetableForm();$("vegetableDialog").showModal()}else $("vegetableDialog").close();alertBox(msg,"success");await loadItems()}catch(x){alertBox(x.message||"Could not save vegetable.","error")}finally{b.disabled=false;b.textContent="Save Vegetable"}}
...
function wire(){
 ...
 $("itemMode").addEventListener("change",()=>{const packet=$("itemMode").value==="packet";$("itemPackSize").disabled=!packet;$("countUnitWrap").classList.toggle("hidden",!packet);$("packSizeWrap").classList.toggle("hidden",!packet);if(!packet){$("itemPackSize").value="";$("itemCountUnit").value=""}});
