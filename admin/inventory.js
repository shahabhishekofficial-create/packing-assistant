...
async function saveVegetable(e,keepOpen=false){e?.preventDefault();const p=vegetablePayload(),errors=validateVegetable(p,S.items);$("vegetableErrors").innerHTML=errors.map(x=>"<div>"+esc(x)+"</div>").join("");if(errors.length)return;const b=$("vegetableSaveBtn");b.disabled=true;b.textContent="Saving…";try{const{data,error}=await db.rpc("inv_v2_save_item",{p_session_token:token(),p_item_id:S.editId,p_payload:p,p_import_id:null});if(error)throw error;const msg=S.editId?"Vegetable updated.":"Vegetable saved.";if(keepOpen){resetVegetableForm();$("vegetableDialog").showModal()}else $("vegetableDialog").close();alertBox(msg,"success");await loadItems()}catch(x){alertBox(x.message||"Could not save vegetable.","error")}finally{b.disabled=false;b.textContent="Save Vegetable"}}
