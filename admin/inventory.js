...
function validateImport(rows){
 ...
 const p={section:norm(r.section),name:norm(r.name),category:norm(r.category),base_uom:norm(r.base_uom),count_mode:norm(r.count_mode).toLowerCase(),default_pack_size:r.default_pack_size===""?null:Number(r.default_pack_size),barcodes:splitPipe(r.barcodes),no_barcode:/^true$/i.test(r.no_barcode),aliases:splitPipe(r.aliases),brand:norm(r.brand)||null};let e=[];
 if(!required.slice(0,9).every(k=>Object.prototype.hasOwnProperty.call(r,k)))e.push("Template columns are incomplete.");
...
function wire(){
 ...
 $("saveItemBtn").onclick=saveItem;$("dialogFetchBtn").onclick=()=>lookup($("itemBarcodes").value.split("|")[0],$("dialogFetchBtn"),$("dialogFetchStatus"));
