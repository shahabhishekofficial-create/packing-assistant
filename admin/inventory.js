...
async function saveVegetable(e,keepOpen=false){e?.preventDefault();const p=vegetablePayload(),errors=validateVegetable(p,S.items);$("vegetableErrors").innerHTML=errors.map(x=>"<div>"+esc(x)+"</div>").join("");if(errors.length)return;const b=$("vegetableSaveBtn");b.disabled=true;b.textContent="Saving…";try{const{data,error}=await db.rpc("inv_v2_save_item_v3",{p_session_token:token(),p_item_id:S.editId,p_payload:p,p_import_id:null});if(error)throw error;const msg=S.editId?"Vegetable updated.":"Vegetable saved.";if(keepOpen){resetVegetableForm();$("vegetableDialog").showModal()}else $("vegetableDialog").close();alertBox(msg,"success");await loadItems()}catch(x){alertBox(x.message||"Could not save vegetable.","error")}finally{b.disabled=false;b.textContent="Save Vegetable"}}
...
async function saveItem(e,keepOpen=false){e?.preventDefault();const p=payloadFromForm(),errors=validateItem(p,S.items);$("fieldErrors").innerHTML=errors.length?errors.map(x=>"<div>"+esc(x)+"</div>").join(""):"";if(errors.length)return;const b=$("saveItemBtn");b.disabled=true;b.textContent="Saving…";try{const {data,error}=await db.rpc("inv_v2_save_item_v3",{p_session_token:token(),p_item_id:S.editId,p_payload:p,p_import_id:null});if(error)throw error;if(keepOpen){const section=p.section;resetForm();$("itemDialog").showModal();}else $("itemDialog").close();alertBox(S.editId?"Item updated.":"Item saved.","success");await loadItems()}catch(x){alertBox(x.message||"Could not save item.","error")}finally{b.disabled=false;b.textContent="Save Item"}}
async function deactivate(id){if(!confirm("Deactivate this item? It will remain in history and cannot be deleted."))return;try{const {error}=await db.rpc("inv_v2_save_item_v3",{p_session_token:token(),p_item_id:id,p_payload:{operation:"deactivate"},p_import_id:null});if(error)throw error;alertBox("Item deactivated.","success");await loadItems()}catch(e){alertBox(e.message||"Could not deactivate item.","error")}}
