...
async function createLiveOrder(rows){
  ...
  try {
    ...
    if (error) {
      throw new Error(`Supabase create_order failed: ${error.message || JSON.stringify(error)}`);
    }
    ...
    // Immediately retrieve the order we just created.
...
$("createOrderBtn").onclick=async()=>{
  try{
    ...
    console.error("Create Live Order:", err);
    alert("Create Live Order failed:\n\n" + (err?.message || String(err)));
  }
  finally{
