...
async function loadDriverAdminDashboard(){
  ...
  try{
    ...
    try{const r=await fetch(window.SUPABASE_CONFIG.url+"/functions/v1/driver-api",{method:"POST",headers:{"apikey":window.SUPABASE_CONFIG.key,"Content-Type":"application/json"},body:JSON.stringify({action:"admin_driver_dashboard",admin_session:window.PA_ADMIN_SESSION,preset,from_date:from,to_date:to}),signal:controller.signal});
      const d=await r.json();if(!r.ok||!d.ok)throw new Error(d.message||"Could not load delivery dashboard");renderDriverDashboard(d);
    }finally{clearTimeout(timeout)}
  }catch(e){box.innerHTML='<div class="hint">Could not load delivery dashboard: '+esc(e.name==="AbortError"?"Timed out. Tap Refresh to retry.":e.message)+'</div>';}
...
document.getElementById("menuConfiguration")?.addEventListener("click",showConfiguration);const teamState={type:"warehouse_staff",rows:[],editId:null};
function ensureTeamForm(){if($("teamMemberDialog"))return;const d=document.createElement("dialog");d.id="teamMemberDialog";d.className="settingsDialog";d.innerHTML='<form method="dialog" id="teamMemberForm"><div class="settingsDialogHead"><div><span class="eyebrow">TEAM CREDENTIALS</span><h3 id="teamMemberTitle">Add Team Member</h3><p class="hint">PIN/password values are never displayed after saving.</p></div><button value="cancel" class="secondary">✕</button></div><label>Name*<input id="teamMemberName" required autocomplete="off"></label><label id="teamLoginWrap">Login name*<input id="teamMemberLogin" autocomplete="off"></label><label>PIN* for new member / leave blank to keep existing<input id="teamMemberPin" type="password" inputmode="numeric" maxlength="6" autocomplete="new-password"></label><label id="teamPaymentWrap">Payment confirmation password (optional)<input id="teamMemberPayment" type="password" autocomplete="new-password"></label><p id="teamMemberError" class="adminAuthError"></p><menu><button value="cancel" class="secondary">Cancel</button><button id="teamMemberSave" value="default" class="primary">Save</button></menu></form></dialog>';document.body.appendChild(d);$("teamMemberForm").addEventListener("submit",saveTeamMember)}
async function loadTeam(){const box=$("teamList");if(!box)return;box.innerHTML='<div class="hint">Loading team…</div>';const{data,error}=await db.rpc("inv_admin_get_team",{p_admin_token:window.PA_ADMIN_SESSION});if(error){box.innerHTML='<div class="hint">Could not load team: '+esc(error.message)+'</div>';return}teamState.rows=data||[];renderTeam();loadTeamAudit()}
function renderTeam(){const rows=teamState.rows.filter(x=>x.member_type===teamState.type);$("teamList").innerHTML=rows.length?rows.map(x=>'<div class="teamRow"><div><b>'+esc(x.name)+'</b><span>'+(x.member_type==="driver"?esc(x.login_name||"No login"):"Warehouse inventory access")+'</span></div><div class="teamRowActions"><span class="statusPill '+(x.active?"on":"off")+'">'+(x.active?"Active":"Inactive")+'</span><button class="secondary teamEdit" data-id="'+x.member_id+'">Edit</button><button class="'+(x.active?"dangerBtn":"secondary")+' teamToggle" data-id="'+x.member_id+'">'+(x.active?"Deactivate":"Activate")+'</button></div></div>').join(""):'<div class="hint">No team members yet.</div>';document.querySelectorAll(".teamEdit").forEach(b=>b.onclick=()=>openTeamMember(b.dataset.id));document.querySelectorAll(".teamToggle").forEach(b=>b.onclick=()=>toggleTeamMember(b.dataset.id))}
async function loadTeamAudit(){const{data,error}=await db.rpc("inv_admin_get_team_audit",{p_admin_token:window.PA_ADMIN_SESSION});if(error){$("teamAuditList").innerHTML='<div class="hint">Could not load audit log.</div>';return}$("teamAuditList").innerHTML=(data||[]).slice(0,30).map(x=>'<div class="teamAuditRow"><b>'+esc(x.action)+'</b><span>'+esc(x.member_type.replace("_"," "))+' · '+new Date(x.created_at).toLocaleString("en-IN")+'</span></div>').join("")||'<div class="hint">No credential changes recorded.</div>}
function openTeamMember(id=null){ensureTeamForm();teamState.editId=id;const x=id?teamState.rows.find(i=>i.member_id===id):null;$("teamMemberTitle").textContent=id?"Edit Team Member":"Add Team Member";$("teamMemberName").value=x?.name||"";$("teamMemberLogin").value=x?.login_name||"";$("teamMemberPin").value="";$("teamMemberPayment").value="";$("teamMemberError").textContent="";$("teamLoginWrap").classList.toggle("hidden",teamState.type!=="driver");$("teamPaymentWrap").classList.toggle("hidden",teamState.type!=="driver");$("teamMemberPin").required=!id;$("teamMemberDialog").showModal();$("teamMemberName").focus()}
async function saveTeamMember(e){e.preventDefault();const name=$("teamMemberName").value.trim(),login=$("teamMemberLogin").value.trim(),pin=$("teamMemberPin").value.trim(),payment=$("teamMemberPayment").value;if(!name)return $("teamMemberError").textContent="Name is required.";if(teamState.type==="driver"&&!login)return $("teamMemberError").textContent="Login name is required.";if(pin&&!/^\d{4,6}$/.test(pin))return $("teamMemberError").textContent="PIN must be 4–6 digits.";const b=$("teamMemberSave");b.disabled=true;b.textContent="Saving…";try{const{error}=await db.rpc("inv_admin_save_team_member",{p_admin_token:window.PA_ADMIN_SESSION,p_member_type:teamState.type,p_member_id:teamState.editId||null,p_name:name,p_login_name:teamState.type==="driver"?login:null,p_pin:pin||null,p_payment_password:teamState.type==="driver"?(payment||null):null});if(error)throw error;$("teamMemberDialog").close();await loadTeam();alert("Team member saved successfully.")}catch(e){$("teamMemberError").textContent=e.message||"Could not save team member."}finally{b.disabled=false;b.textContent="Save"}}
async function toggleTeamMember(id){const x=teamState.rows.find(i=>i.member_id===id);if(!x)return;const active=!x.active;if(!active&&!confirm("Deactivate "+x.name+"? Existing historical records will remain."))return;try{const{error}=await db.rpc("inv_admin_set_team_active",{p_admin_token:window.PA_ADMIN_SESSION,p_member_type:x.member_type,p_member_id:id,p_active:active});if(error)throw error;await loadTeam()}catch(e){alert("Could not change team member: "+e.message)}}
function openManageTeam(){if(!window.PA_ADMIN_SESSION)return window.PA_REAUTH_ADMIN?.();$("manageTeamDialog").showModal();loadTeam()}
document.getElementById("menuManageTeam")?.addEventListener("click",()=>{adminMenu?.classList.add("hidden");openManageTeam()});document.getElementById("closeManageTeam")?.addEventListener("click",()=>$("manageTeamDialog")?.close());document.getElementById("manageTeamDashboardBack")?.addEventListener("click",()=>{$("manageTeamDialog")?.close();showAdminDashboard()});document.getElementById("addTeamMember")?.addEventListener("click",()=>openTeamMember());document.getElementById("refreshTeam")?.addEventListener("click",()=>loadTeam());document.querySelectorAll(".teamTab").forEach(b=>b.addEventListener("click",()=>{teamState.type=b.dataset.teamType;document.querySelectorAll(".teamTab").forEach(x=>x.classList.toggle("active",x===b));$("addTeamMember").textContent=teamState.type==="driver"?"+ Add Driver":"+ Add Staff";renderTeam()}));
