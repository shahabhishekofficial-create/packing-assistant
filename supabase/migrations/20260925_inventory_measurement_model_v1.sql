...
create or replace function public.inv_v2_save_item_v5(
 p_session_token text,p_item_id uuid default null,p_payload jsonb default '{}'::jsonb,p_import_id uuid default null
) returns jsonb
language plpgsql security definer set search_path=''
as $$
declare result jsonb; v_id uuid; v_legacy uuid; v_mode text;
 v_unit text; v_factor numeric; v_uom text; v_sec text; x jsonb;
begin
 if not public.inv_require_admin(p_session_token) then raise exception 'Unauthorized'; end if;
 v_mode=lower(public.inv_v2_normalize_text(p_payload->>'count_mode'));
 v_sec=public.inv_v2_normalize_text(p_payload->>'section');
 v_uom=public.inv_v2_normalize_text(p_payload->>'base_uom');
 v_unit=public.inv_v2_normalize_text(p_payload->>'count_unit');
 v_factor=nullif(trim(p_payload->>'count_to_base'),'')::numeric;
 if v_sec='vegetable' then
   v_unit=case when v_uom='kg' then 'kg' else 'pcs' end;
   v_factor=1;
 elsif v_mode='unit' then
   v_unit=case when v_uom='kg' then 'kg' when v_uom='g' then 'g' when v_uom='L' then 'L' when v_uom='ml' then 'ml' else 'pcs' end;
   v_factor=1;
 elsif v_mode='packet' then
   if v_unit='' then raise exception 'Count unit is required for Packet mode'; end if;
   if v_factor is null or v_factor<=0 then raise exception 'Quantity per count unit must be greater than 0'; end if;
 end if;
 p_payload=jsonb_set(jsonb_set(p_payload,'{count_unit}',to_jsonb(v_unit),true),'{count_to_base}',to_jsonb(v_factor),true);
 result:=public.inv_v2_save_item_v4(p_session_token,p_item_id,p_payload,p_import_id);
 v_id=(result->>'item_id')::uuid;
 if v_id is not null then
   select legacy_item_id into v_legacy from public.inv_v2_items where id=v_id;
   update public.inv_v2_items set count_unit=v_unit,count_to_base=v_factor,updated_at=now() where id=v_id;
   update public.inv_items set count_unit=v_unit,count_to_base=v_factor where id=v_legacy;
 end if;
 return result;
end $$;
