...
let cameraStream=null,cameraVideo=null,cameraDialog=null,cameraLoop=false;
function closeCamera(){if(cameraDialog){cameraDialog.close();cameraDialog.remove();cameraDialog=null}cameraLoop=false}
async function scanBarcode(){
  ...
  try{
    ...
    if(!cameraDialog){
      cameraDialog=document.createElement("dialog");
      cameraDialog.className="invCameraDialog";
      cameraDialog.innerHTML='<div class="invCameraHead"><div><b>Scan barcode</b><small>Align the barcode inside the frame.</small></div><button type="button" class="invCameraClose">Cancel</button></div><div class="invCameraViewport"><video autoplay muted playsinline></video><div class="invScanFrame"></div></div>';
      document.body.appendChild(cameraDialog);
      cameraVideo=cameraDialog.querySelector("video");
      cameraDialog.querySelector(".invCameraClose").onclick=closeCamera;
      cameraDialog.addEventListener("cancel",e=>{e.preventDefault();closeCamera()});
    ...
    cameraLoop=true;
    const loop=async()=>{
      if(!cameraLoop||!cameraVideo||!cameraDialog?.open)return;
      try{
